...
def update_motor_settings(new_settings):
    ...
    # Обновляем время последнего шага для корректного расчета ускорения
    ...
    # Текущие скорости не обнуляются, что позволяет применять
    # изменения скорости в реальном времени без остановки мотора.
