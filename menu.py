...
STEP = 500
TOUCH_TOLERANCE = 20  # Допустимое отклонение от центра кнопки

# Список моторов и параметров
...
# Параметры для каждого мотора
PARAMETERS_BY_MOTOR = [
    ["speed", "acceleration", "distance"],  # Руль
    ["speed", "acceleration", "distance"],  # Газ
    ["speed", "acceleration", "distance"],  # Тормоз
    ["speed", "acceleration", "distance_R", "distance_D"]  # АКПП
]
...
class Menu:
    def __init__(self, motor_settings, akpp_center):
        ...
        # Предварительное создание изображений
        self.main_menu_image = self.create_main_menu_image()
        self.param_menu_images = {}
        self.update_screen(self.main_menu_image)

    def find_touch_device(self):
        """Поиск устройства тачскрина"""
        try:
            devices = [InputDevice(path) for path in evdev.list_devices()]
            for device in devices:
                if "touchscreen" in device.name.lower() or "ft5x06" in device.name.lower():
                    print(f"[INFO] Используем тачскрин: {device.name}")
                    device.grab()  # Эксклюзивный доступ к устройству
                    return device
            
            if devices:
                print(f"[WARNING] Используем первое доступное устройство: {devices[0].name}")
                devices[0].grab()
                return devices[0]
                
        except Exception as e:
            print(f"[ERROR] Не удалось найти устройство ввода: {e}")
        
        return None

    def create_main_menu_image(self):
        """Создание изображения главного меню"""
    ...
    def create_parameter_menu_image(self, motor_index, param_index):
        """Создание изображения меню параметров"""
        image = Image.new("RGB", (WIDTH, HEIGHT), self.BACKGROUND_COLOR)
        draw = ImageDraw.Draw(image)
    ...
    def draw_motor_selection(self):
        ...
        print("[DEBUG] Отрисовка главного меню")
        self.is_in_main_menu = True
        self.update_screen(self.main_menu_image)

    def draw_metal_button(self, draw, x, y, radius):
        """Рисуем кнопки с металлическим градиентом"""
    ...
    def draw_parameter_menu(self):
        """Переключение на меню параметров"""
        print(f"[DEBUG] Переход в меню параметров для мотора: {MOTORS[self.current_motor_index]}")
        self.is_in_main_menu = False
        
        # Создаем или используем кэшированное изображение
    ...
    def is_touch_in_circle(self, x, y, center_x, center_y, radius):
        """Проверяем, попадает ли точка в окружность с учетом допуска"""
        distance = math.sqrt((x - center_x) ** 2 + (y - center_y) ** 2)
        return distance < (radius + TOUCH_TOLERANCE)

    def process_touch(self, x, y):
        """Обработка касания из очереди"""
        ...
        print(f"[DEBUG] Обработка касания: {x}, {y}")
        
        # Нормализация координат для разных разрешений
        if WIDTH == 800 and HEIGHT == 480:
            # Для стандартного разрешения ничего не делаем
            pass
        else:
            # Масштабирование координат
            x = int(x * WIDTH / 800)
            y = int(y * HEIGHT / 480)
            print(f"[DEBUG] Масштабированные координаты: {x}, {y}")
        ...
        if self.is_in_main_menu:
            ...
            if self.is_touch_in_circle(x, y, WIDTH // 2, HEIGHT // 2, BUTTON_RADIUS_LARGE):
                print("[DEBUG] Нажата кнопка КАЛИБРОВКА")
    ...
    def adjust_parameter(self, delta):
        """Изменение параметра с обновлением экрана"""
    ...
    def start_calibration(self):
        ...
        print("[INFO] Начало процесса калибровки...")
        motor.calibrate_motors()
        print("[INFO] Калибровка завершена.")
    ...
    def save_settings(self):
        ...
        self.apply_motor_settings()
        print("[DEBUG] Настройки сохранены и применены.")

    def apply_motor_settings(self):
        """Применение новых настроек к моторам"""
    ...
    def cleanup(self):
        ...
        print("[DEBUG] Очистка экрана и завершение программы.")
        image = Image.new("RGB", (WIDTH, HEIGHT), (0, 0, 0))
        self.update_screen(image)
        if self.touch_device:
            self.touch_device.ungrab()  # Освобождаем устройство
        self.running = False
    ...
    def update_screen(self, image):
        """Обновление экрана с буферизацией"""
        try:
            # Конвертируем изображение в сырые данные для framebuffer
            ...
            print(f"[ERROR] Ошибка при обновлении экрана: {e}")

    def touch_listener_thread(self):
        """Поток для прослушивания сенсорных событий"""
        print("[DEBUG] Запуск потока обработки сенсора")
        last_x = None
        last_y = None
                ...
                if events:
                    ...
                    for event in events:
                        if event.type == ecodes.EV_ABS:
                            ...
                            if event.value == 1:  # Начало касания
                                touch_detected = True
                            elif event.value == 0:  # Конец касания
                                last_x = None
                                last_y = None
                    
                    if touch_detected and last_x is not None and last_y is not None:
                        self.touch_queue.put((last_x, last_y))
                        # Очищаем координаты после обработки
                        last_x = None
                        last_y = None
            except BlockingIOError:
                time.sleep(0.001)
            except Exception as e:
                print(f"[ERROR] Ошибка в потоке сенсора: {e}")
                time.sleep(0.1)

    def run(self):
        print("[DEBUG] Запуск меню...")
        self.draw_motor_selection()
        
        # Запускаем поток для обработки сенсора
        if self.touch_device:
        ...
        while self.running:
            try:
                # Обрабатываем только последнее событие в очереди
                # (очищаем очередь, сохраняя только последнее событие)
                last_touch = None
...
# Основной код для запуска (остается без изменений)
if __name__ == "__main__":
    def signal_handler(sig, frame):
        print("Прерывание программы... Завершаем корректно.")
        menu.save_settings()
        menu.cleanup()
        os._exit(0)
